local Translations = {
    ["Auto Farm"] = "自动农场",
    ["AUTOMATION"] = "自动化",
    ["Auto Open (Crate + More)"] = "自动打开(板条箱+更多)",
    ["Auto Loot (Item)"] = "自动拾取(变卖物)",
    ["Auto Loot (Gear)"] = "自动拾取(道具)",
    ["Auto Rescue (NPC)"] = "自动营救(NPC)",
    ["PLAYER"] = "玩家修改",
    ["Always (SafeZone)"] = "始终(安全区)",
    ["Anti Trap (Monster)"] = "防陷阱(怪物)",
